...
pub async fn rbac_middleware(req: Request, next: Next) -> Result<Response, ApiError> {
    ...
    if method == axum::http::Method::POST || method == axum::http::Method::PUT || method == axum::http::Method::PATCH || method == axum::http::Method::DELETE {
        let role_str = req.headers().get("x-role")
