...
/// DELETE /api/v1/assistance-requests/{id}. Dependent helper allocations are
/// removed by the Postgres FK cascade; in-memory allocations are reconciled
/// later.
pub async fn delete(
    State(state): State<AppState>,
    Path(id): Path<Uuid>,
...
async fn delete_postgres(pool: &sqlx::PgPool, id: Uuid) -> Result<(), ApiError> {
    let result = sqlx::query("DELETE FROM assistance_requests WHERE id = $1")
        .bind(id)
        .execute(pool)
        .await?;
    ...
    fn from(row: AssistanceRequestRow) -> Self {
        AssistanceRequest {
            ...
            status: serde_json::from_str(&row.status)
                ...
                .unwrap_or(AssistanceStatus::Pending),
