...
function AdminLayout() {
	// Admin tree depends on Zustand stores that call React.useState/useCallback
	// during SSR — TanStack Start runs SSR and Zustand's React-context-based
	// `useSyncExternalStore` shim crashes when there's no dispatcher. Render
	// nothing during SSR; the full tree mounts on the client.
...
function AdminShell() {
	const { simStatus, wsConnected, fetchData, toggleSimulation } =
		useAdminStore();
	const routerState = useRouterState();

	// WS drives live updates; falls back to 10s polling when disconnected
	// so the admin panel is never stale.
	useWebSocket(wsUrl());
